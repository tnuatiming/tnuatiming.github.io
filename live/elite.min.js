...
function createLiveTable(a){var e="0",f="1",h="1",g="0",u="0",w="0",v="0",d=0,k=0;d=0;var b=[];k=[];var c=[],D=0,B="99999999999",z=[],p="&nbsp;",l=0;d=0;var m="";var y=6,C="no",E="",F="",G="";a=a.split("<table");a[1]=a[1].substring(a[1].indexOf("<tr"),a[1].lastIndexOf("</tr>")+5);m=a[1].split("\n");var t=a[0].split("\n"),r=document.createElement("div");r.innerHTML=t[0];var n=r.textContent||r.innerText||"";var A=n.split("-")[1].trim();r=t[1].split(":");r=("0"+r[0].slice(-2).trim()).slice(-2)+":"+r[1];
var q={};eventName!=n&&(doNotShowInTicker=[],ticker=[],firstPlace=flag="",tickerBestTime="-",categoryBestTimePrevious={});eventName=n;sessionStorage.setItem("eventName",eventName);a[0].includes("_GreenFlag")?("0"==firstPass&&"green"!=flag&&ticker.push(r+" - \u05d6\u05d9\u05e0\u05d5\u05e7/\u05d3\u05d2\u05dc \u05d9\u05e8\u05d5\u05e7 "+A),flag="green"):a[0].includes("_CheckeredFlag")?("0"==firstPass&&"checkered"!=flag&&ticker.push(r+" - \u05e1\u05d9\u05d5\u05dd "+A),flag="checkered"):a[0].includes("_Stop")?
("0"==firstPass&&"checkered"!=flag&&ticker.push(r+" - \u05e1\u05d9\u05d5\u05dd"),flag="checkered"):a[0].includes("_RedFlag")?("0"==firstPass&&"red"!=flag&&ticker.push(r+" - \u05d3\u05d2\u05dc \u05d0\u05d3\u05d5\u05dd"),flag="red"):a[0].includes("_YellowFlag")?("0"==firstPass&&"yellow"!=flag&&ticker.push(r+" - \u05d3\u05d2\u05dc \u05e6\u05d4\u05d5\u05d1"),flag="yellow"):flag="";if(t[0].includes("\u05d8\u05e1\u05d8")||t[0].includes("\u05e1\u05e4\u05d9\u05d9\u05e9\u05dc"))e=g="1",f="0";t[0].includes("2 \u05d4\u05e7\u05e4\u05d5\u05ea")&&
...
m[n].substring(m[n].indexOf(' id="')+4).split('"')[1],m[n].substring(m[n].indexOf(">")+1,m[n].lastIndexOf("<")),b.push(A);else if(m[n].includes("OddRow")||m[n].includes("EvenRow"))l=1;else if(m[n].includes("</tr>")&&1==l)l=0,"undefined"==typeof q[k.Id_Categorie]&&(q[k.Id_Categorie]=[Number(99999999),"-","-"]),q[k.Id_Categorie][0]>timeString2ms(k.Id_MeilleurTour)&&"-"!=k.Id_MeilleurTour&&(q[k.Id_Categorie]=[Number(timeString2ms(k.Id_MeilleurTour)),k.Id_Numero,k.Id_Nom]),c.push(k),k=[],d=0,C="no";else if(m[n].includes("<td ")&&
1==l){m[n].includes("(C)")&&(C="yes");if("Id_Position"==b[d]||"Id_NbTour"==b[d]||"Id_TpsCumule"==b[d])k[b[d]+"_Penalty"]="yes"==C?"1":"0";k[b[d]]="undefined"!=typeof m[n]&&"1"==cleanResults?m[n].substring(m[n].indexOf(">")+1,m[n].lastIndexOf("<")).replace("(C) ",""):"undefined"!=typeof m[n]?m[n].substring(m[n].indexOf(">")+1,m[n].lastIndexOf("<")).replace("(C) ",""):"-";m[n].includes("BestTimeOverall")&&"Id_TpsTour"==b[d]?z[k.Id_Numero]=" BestTimeOverall":m[n].includes("BestTime")&&"Id_TpsTour"==
b[d]?z[k.Id_Numero]=" BestTime":"Id_TpsTour"==b[d]&&(z[k.Id_Numero]="");"Id_Categorie"!=b[d]||"-"!=k[b[d]]&&""!=k[b[d]]&&"&nbsp;"!=k[b[d]]&&"undefined"!=typeof k[b[d]]||(k[b[d]]="&nbsp;");"1"==e&&("Id_TpsTour1"==b[d]||"Id_TpsTour2"==b[d]||"Id_TpsTour3"==b[d]||"Id_TpsTour4"==b[d]||"Id_TpsTour5"==b[d]||"Id_TpsTour6"==b[d])&&"-"!=k[b[d]]&&timeString2ms(k[b[d]])<=timeString2ms(B)&&(B=k[b[d]],D=k.Id_Numero);d+=1}"yes"==useCategory&&c.sort(function(a,b){return("&nbsp;"==a.Id_Categorie)-("&nbsp;"==b.Id_Categorie)||
a.Id_Categorie.localeCompare(b.Id_Categorie)||a.Id_PositionCategorie-b.Id_PositionCategorie});a+='\n<div id="liveTable"><table class="'+tableClass+'line_color">\n';for(b=0;b<c.length;b++){"no"==useCategory&&"-"!=c[b].Id_Ecart1er&&("00:"==c[b].Id_Ecart1er.toString().substring(0,3)&&(c[b].Id_Ecart1er=c[b].Id_Ecart1er.substr(3)),"0"==c[b].Id_Ecart1er.toString().substring(0,1)&&c[b].Id_Ecart1er.includes(":")&&(c[b].Id_Ecart1er=c[b].Id_Ecart1er.substr(1)));"yes"==useCategory&&"-"!=c[b].Id_Ecart1erCategorie&&
("00:"==c[b].Id_Ecart1erCategorie.toString().substring(0,3)&&(c[b].Id_Ecart1erCategorie=c[b].Id_Ecart1erCategorie.substr(3)),"0"==c[b].Id_Ecart1erCategorie.toString().substring(0,1)&&c[b].Id_Ecart1erCategorie.includes(":")&&(c[b].Id_Ecart1erCategorie=c[b].Id_Ecart1erCategorie.substr(1)));"undefined"!=typeof c[b].Id_TpsCumule&&"-"!=c[b].Id_TpsCumule&&("00:"==c[b].Id_TpsCumule.toString().substring(0,3)&&(c[b].Id_TpsCumule=c[b].Id_TpsCumule.substr(3)),"0"==c[b].Id_TpsCumule.toString().substring(0,1)&&
c[b].Id_TpsCumule.includes(":")&&(c[b].Id_TpsCumule=c[b].Id_TpsCumule.substr(1)));if(c[b].Id_TpsTour1&&"1"==e)for(d=k=y;-1<d;d--)if(c[b]["Id_TpsTour"+d]&&"-"!=c[b]["Id_TpsTour"+d]||0==d){for(l=1;l<=k;l++)c[b]["Id_lap"+l]=0<d?c[b]["Id_TpsTour"+d]:"-",d--;d=-1}l='<tr class="rnkh_bkcolor">\n';"1"!=cleanResults&&(l+='<th class="rnkh_font Id_Arrow"></th>\n');l+='<th class="rnkh_font Id_Position">\u05de\u05e7\u05d5\u05dd</th>\n';l+='<th class="rnkh_font Id_Numero">\u05de\u05e1\u05e4\u05e8</th>\n';l+='<th class="rnkh_font Id_Nom">\u05e9\u05dd</th>\n';
...
"no"==useCategory&&(d=Number(c[b].Id_Position));c[b].Id_PositionCategorie&&"yes"==useCategory&&(d=Number(c[b].Id_PositionCategorie));m="";0<d&&0<=k&&("-"!=c[b].Id_TpsTour||0<c[b].Id_NbTour||c[b].Id_Image.includes("_TrackPassing"))&&(positionArray[k]&&(0<c[b].Id_NbTour||"1"==v&&"-"!=c[b].Id_TpsTour)&&(positionArray[k]<d?(c[b].Id_Arrow='<img class="postionChanged" src="Images/_MinusPosition.svg" alt="lost places">',m="lostPosition "):positionArray[k]>d&&(c[b].Id_Arrow='<img class="postionChanged" src="Images/_PlusPosition.svg" alt="gained places">',
m="gainedPosition ","no"==useCategory&&1<d&&"???"!=c[b].Id_Nom&&3>=c[b].Id_Position?ticker.push(r+" - "+c[b].Id_Nom+" ("+c[b].Id_Numero+") \u05e2\u05dc\u05d4 \u05dc\u05de\u05e7\u05d5\u05dd "+d):1<Object.keys(q).length&&"yes"==useCategory&&"???"!=c[b].Id_Nom&&3>=c[b].Id_PositionCategorie&&"undefined"!=c[b].Id_Categorie?ticker.push(r+" - "+c[b].Id_Nom+" ("+c[b].Id_Numero+") \u05e2\u05dc\u05d4 \u05dc\u05de\u05e7\u05d5\u05dd "+d+" \u05d1\u05e7\u05d8\u05d2\u05d5\u05e8\u05d9\u05d4 "+c[b].Id_Categorie):
"yes"==useCategory&&"???"!=c[b].Id_Nom&&3>=c[b].Id_PositionCategorie&&"undefined"!=c[b].Id_Categorie&&ticker.push(r+" - "+c[b].Id_Nom+" ("+c[b].Id_Numero+") \u05e2\u05dc\u05d4 \u05dc\u05de\u05e7\u05d5\u05dd "+d))),positionArray[k]=d);c[b].Id_NbTour&&(d=c[b].Id_NbTour,d!=lapsArray[k]&&""==m&&"0"==firstPass&&(m="unChanged "),lapsArray[k]=d);""!=m&&"unChanged "!=m||!c[b].Id_Image.includes("_TrackPassing")&&"1"!=c[b].Id_Canal||c[b].Id_Image.includes("_Status")||(c[b].Id_Arrow='<img class="postionSame" src="Images/_TrackPassing.svg" alt="same places">');
...
firstPass="0";return a}function timeString2ms(a,e){return a=a.split("."),e=1*a[1]||0,a=a[0].split(":"),e+1E3*(a[2]?3600*a[0]+60*a[1]+1*a[2]:a[1]?60*a[0]+1*a[1]:1*a[0])}function ms2TimeString(a,e,f,h,g){return e=a%1E3,f=a/1E3%60|0,h=a/6E4%60|0,g=a/36E5%24|0,(g?(10>g?"0"+g:g)+":":"")+(10>h?0:"")+h+":"+(10>f?0:"")+f+"."+(100>e?10>e?"00":0:"")+e};
