...
var DataGridRenderer = {
  ...
  html: function (dataGrid, headerNames, headerTypes, indent, newLine) {
    ...
    //begin render loop
    outputText += '<table ALIGN="CENTER" CELLSPACING=1 class="line_color">'+newLine;
    outputText += indent+indent+'<tr class="rnkh_bkcolor">'+newLine;

    for (var j=0; j < numColumns; j++) {
      outputText += indent+indent+indent+'<td class="rnkh_font" align="CENTER">';
    ...
    for (var i=0; i < numRows; i++) {
      ...
      outputText += indent+indent+'<tr class="rnk_bkcolor">'+newLine;
      for (var j=0; j < numColumns; j++) {
        outputText += indent+indent+indent+'<td class="rnk_font" align="CENTER">';
        outputText += row[j]
        outputText += '</td>'+newLine
      };
      outputText += indent+indent+"</tr>"+newLine;
